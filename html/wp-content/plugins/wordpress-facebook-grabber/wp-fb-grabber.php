...
function fbAlbumGrabber($fburl,$album_id){
	//"https://graph.facebook.com/10151886200760752/photos?limit=6&offset=8"
	$maxitem = get_option('max_photo');
	$offset = get_option('photo_offset') ? get_option('photo_offset') : "0";
	$album_id = get_option('album_id') ?  get_option('album_id')  : "10151886200760752" ;
	
	$fburl = "https://graph.facebook.com/".$album_id."/photos?limit=".$maxitem."&offset=".$offset;
		...
		foreach($data->data as $data->item){
			if($count != -1) {
				...
				<p class="fb_feed_title">'.str_replace("-"," ",sanitize_title($data->item->name)).'</p>
					';
					if($data->item->source)
					$retval .='<div class="fb_feed_thumb"><img class="wp_fb_album_grabber_img" src="'.htmlentities($data->item->source).'" alt=" '.str_replace("-"," ",sanitize_title($data->item->name)).'" title=" '.str_replace("-"," ",sanitize_title($data->item->name)).' " /></div>';
